    ...
    <div className="border-2 border-indigo-200 rounded-lg p-4">
      ...
      {nf === '2NF' && (data.partialDependencies?.length ?? 0) > 0 && (
        <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
          <div className="font-semibold text-yellow-900 mb-1">Partial Dependencies Found:</div>
      ...
      {nf === '3NF' && (data.transitiveDependencies?.length ?? 0) > 0 && (
        <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
          <div className="font-semibold text-yellow-900 mb-1">Transitive Dependencies Found:</div>
      ...
      <div className="space-y-3">
        ...
        {data.tables.map((table, i) => (
          <div key={i} className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200">
            <div className="font-bold text-gray-900 mb-2 text-lg">{table.name}</div>
            
            <div className="space-y-1">
              <div className="text-sm break-words">
                <span className="font-semibold text-gray-700">Attributes:</span>
                <span className="ml-2 text-gray-800">{table.attributes.join(', ')}</span>
              </div>
              
              <div className="text-sm break-words">
                <span className="font-semibold text-indigo-700">Primary Key:</span>
                <span className="ml-2 text-indigo-800 font-mono">{table.primaryKey.join(', ')}</span>
              </div>
              
              {table.foreignKeys && table.foreignKeys.length > 0 && (
                <div className="text-sm">
                  <span className="font-semibold text-purple-700">Foreign Keys:</span>
      ...
      {data.notes && data.notes.length > 0 && (
        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
          ...
          {data.notes.map((note, i) => (
            <div key={i} className="text-sm text-blue-800">• {note}</div>
          ))}
        </div>
      )}
    </div>
